...
const mutations = {
  ...
  [GETALLRESOURCESLIST] (state, payload) {
    ...
    for (let i = 0, j = payload.length; i < j; i++) {
      let option = payload[i].available_operations
      if (!option || typeof option !== 'string') {
        break
      }
      payload[i].available_operations = option.split(' ')
      payload[i].original_operations = payload[i].available_operations
    }
...
const actions = {
  ...
  getAllRoleList ({commit}, payload) {
    ...
    .then((response) => {
      commit(GETALLROLELIST, response.data.roles)
      return response
    })
    .catch((error) => {
